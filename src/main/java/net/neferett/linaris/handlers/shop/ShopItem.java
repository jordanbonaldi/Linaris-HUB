package net.neferett.linaris.handlers.shop;
...
import net.neferett.linaris.handlers.shop.guis.BuyKitGui;
	...
	public ItemStack getItemUI(Player p) {
		...
		if (firstFree) {
			...
			if (level == getLevelInfos().size()-1) {
				...
				lore.add("§aClic gauche: d§bloquer §e" + next.getPrice() + "Coins");
				lore.add("§bClic droit: voir les niveaux");
			}
		...
		// else {
		// if (level == 0) {
		// lore.add("§cVous ne poss§dez pas,");
