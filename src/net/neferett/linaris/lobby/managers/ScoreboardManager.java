...
import net.neferett.linaris.lobby.utils.RainbowEffect;
import net.neferett.linaris.lobby.utils.ScoreBoardModule;
...
public class ScoreboardManager extends ScoreBoardModule {
	...
	public ScoreboardManager(final Lobby game) {
		super(game);
		...
		if (jump.get(p) == false) {
			...
			for (int i = 1; i < 16; i++)
				if (!lines.containsKey(i)) {
					if (bar.getLine(i) != null) bar.removeLine(i);
				} else if (bar.getLine(i) == null)
					bar.setLine(i, lines.get(i));
				else if (!bar.getLine(i).equals(lines.get(i))) bar.setLine(i, lines.get(i));
		}
