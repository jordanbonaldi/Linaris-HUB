package net.neferett.linaris.lobby.handlers.inventories.magicbox;
...
import net.neferett.linaris.lobby.handlers.inventories.MagicBoxInventory;
...
public class MetamorphosesGui extends GuiScreen {
	...
	public void drawScreen() {
		...
		if (SettingsManager.isEnabled(getPlayer().getName(), Games.LOBBY, "metamorphose-viewself",true))
			setItemLine(ItemStackUtils.createRenamedItemStack(Material.EYE_OF_ENDER, 1, (short)0, "§6Métamorphose §aactivée", null), 6, 5);
		else
			setItemLine(ItemStackUtils.createRenamedItemStack(Material.ENDER_PEARL, 1, (short)0, "§6Métamorphose §cdésactivée", null), 6, 5);
		...
		setItemLine(15, new ItemBuilder(Material.ARROW).setTitle("§fRevenir en arrière").build(), 6, 9);
	}
